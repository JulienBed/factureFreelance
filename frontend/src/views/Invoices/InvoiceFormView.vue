      ...
      <div class="flex justify-between items-center pt-4 border-t border-gray-100">
        <div>
          <button
            ...
            class="inline-flex items-center gap-2 px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
          >
            <svg v-if="downloading" class="h-5 w-5 animate-spin" fill="none" viewBox="0 0 24 24">
            ...
            {{ downloading ? t('invoices.actions.downloading') : t('invoices.actions.downloadPdf') }}
          </button>
        </div>
...
const handleDownloadPdf = async () => {
  ...
  try {
    ...
    if (invoice) {
      await invoicesStore.downloadPdf(invoice.id, invoice.invoiceNumber)
    }
  } catch (err) {
